...
/* guarda o resultado do ultimo experimento automatico (opcao 5 do menu) */
/* -1 indica que o experimento ainda nao foi executado                    */
...
// ----------------------------------------------------------------
// gera um nome aleatorio no formato 8 letras + 3 numeros
// usado so para criar consultas "negativas" (usuarios que NAO
// estao cadastrados) durante o experimento de falsos positivos
// ----------------------------------------------------------------
...
void executar_experimentos() {
    ...
    for (int idx = 0; idx < NUM_CENARIOS; idx++) {
        ...
        /* dimensiona o bloom corretamente para cada cenario usando a formula ideal */
        /* isso garante ~1% de falso positivo independente do volume de dados       */
        int bits_recomendados, funcs_recomendadas;
        bloom_recomendar_dimensoes(n, 0.01, &bits_recomendados, &funcs_recomendadas);

        /* cria estruturas isoladas para este cenario, sem interferir nas globais */
       ...
       /* abre o arquivo de usuarios do cenario atual */
        ...
        // -----------------------------------------------------
        // 1) TEMPO SEM BLOOM
        // busca todos os registros diretamente na hash,
        // sem nenhum filtro pre-consulta
        // -----------------------------------------------------
        ...
        // -----------------------------------------------------
        // 2) TEMPO COM BLOOM
        // consulta o bloom primeiro; so vai na hash se o bloom
        // disser "possivelmente existe" — evita acessos desnecessarios
        // -----------------------------------------------------
        ...
        // -----------------------------------------------------
        // 3) FALSOS POSITIVOS
        // gera nomes que NAO estao cadastrados e conta quantas
        // vezes o bloom erra dizendo que existe (falso positivo)
        // -----------------------------------------------------
        int falsos_pos = 0;
...
// ----------------------------------------------------------------
// exibe as estatisticas do sistema
// ----------------------------------------------------------------
// mostra dados da tabela hash, do filtro de bloom e das consultas
// manuais realizadas pelo usuario via opcao 2 do menu
// tambem exibe o resultado do ultimo experimento automatico,
// se ja tiver sido executado via opcao 5
// ----------------------------------------------------------------
...
if (consultas_realizadas > 0) {
    ...
    /* exibe resultado do ultimo experimento automatico, se disponivel */
